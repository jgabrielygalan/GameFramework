...
function doLogin() {
    ...
    $.post("/auth", {username: user, password:passw}, function(data) {
        if (data.success) {
            ...
            $('#createMatchButton').prop('disabled', false);
...
function getGamesAndUsers() {
    $.get('/games').done(function(games) {
        var div = $('<div/>');
        var select = $('<select id="gameList">');
        $.each(games, function(index, game) {
            select.append("<option value='" + game.rels.uri + "'>" + game.name + "</option>");
        });
        div.append(select);
        $.get('/users').done(function(opponents) {
            var select2 = $('<select id="opponentList">');
            $.each(opponents, function(index, opponent) {
                select2.append("<option value='" + opponent + "'>" + opponent + "</option>");
            });
            div.append(select2);
            var button = $('<button id="createMatch">Create match</button>');
            button.click(createMatch);
            div.append(button);
            $('#content div').empty().append(div);
        });
    });
}

function createMatch() {
    postWithToken($('#gameList').val(), {"opponent": $('#opponentList').val()}).done(function() {
        alert ('Match created');
    });
}
...
$(document).ready(function() {
    ...
    $("#createMatchButton").click(getGamesAndUsers);
});